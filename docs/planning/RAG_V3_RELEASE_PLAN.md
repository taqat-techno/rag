...
Uninstall residue sweep (V15)              9/9     install -> uninstall -> scan finds nothing
Clean install of the built wheel (V01)     4/4     imports, resolves its platform, no watchdog
...
Autostart lifecycle (V03/V04)              20/20   real Task Scheduler, UNELEVATED, both concerns
Validation matrix                          14 pass / 0 fail / 1 manual
```

**Every row's evidence is now reproducible from a committed script.** V13 and
V14 were previously closed by ad-hoc probes that existed nowhere in the
repository, and V01's evidence pointed at `scripts/verify_clean_install.py`,
which was never written. A gate whose evidence cannot be re-run before the next
release is a report, not a gate. The matrix is now regenerated by:

```
scripts/verify_uninstall_residue.py     -> V01 (install phase) + V15
scripts/rehearse_upgrade.py             -> V02
scripts/verify_autostart_lifecycle.py   -> V03 + V04
scripts/verify_storage_recovery.py      -> V12
scripts/verify_corpus_gates.py          -> V13 + V14
scripts/validate_release.py             -> the matrix
```
...
### The "needs elevation" blocker was a product defect

V03 and V04 were recorded as blocked on an elevated session, on the evidence
that `schtasks /create` returns *Access is denied* at both root and folder
scope. That measurement was correct. The conclusion drawn from it was not.

Probing each operation separately on the same standard account:

| operation | result |
|---|---|
| `schtasks /create /sc onlogon /rl limited` (root) | **Access is denied** |
| `schtasks /create /sc onlogon /ru <me>` | **Access is denied** |
| `schtasks /create /sc once` (root) | **works** |
| `Register-ScheduledTask -AtLogOn -User <me>` (root) | **works** |
| `Register-ScheduledTask` into a **new folder** | **works** |

Neither the library root nor folder creation is privileged. `schtasks /sc
onlogon` builds a logon trigger with **no `<UserId>`** — "at logon of *any*
user" — and only an administrator may register that. So the shipped adapter
could not register autostart for a standard account **at all**: a per-user
product that needed elevation to start itself, on the exact account it was
built for. An elevated shell would have made both rows pass and shipped the
defect.

Fixed by registering from a task XML that names the user in the trigger
(`platform/windows.py`), which also settles three defaults `/sc onlogon` never
touched — the task would not start on battery, would be *stopped* on switching
to battery, and would be killed at the default 72-hour execution limit — and
supplies the native `RestartOnFailure` policy the retired watchdog was
imitating by hand. Removal now also prunes the empty task folder, which the
scheduler keeps after its last task is deleted.

**V03/V04: 20 checks, 0 failures** against the real Task Scheduler, unelevated
(`scripts/verify_autostart_lifecycle.py`), covering both concerns: registration
created, bound to this user's logon, no elevation, no interpreted shim, the
recorded command actually executed, removal idempotent, zero residue including
the folder. Unproven, and stated as such: that Windows fires an at-logon
trigger at logon. That is the OS's behaviour, not this product's.

> CI would not have caught this. GitHub's `windows-latest` runner is an
> administrator, so `/sc onlogon` succeeds there. It was visible only on a
> normal account — the configuration every user has.

Two further defects surfaced while closing these rows, both in the same seam:

* **Autostart started the service on the wrong data root.** The registration
  declares `RAG_PROFILE=installed`. A systemd unit and a launchd plist can carry
  an environment; a scheduled task cannot, so the declaration was honoured on
  two platforms and silently discarded on the third — where a source install
  then falls back to the `dev` profile and the service that comes up at login
  serves a *different index* than the one that registered it. One registration,
  two answers, decided by the OS. The profile now travels as `rag service run
  --profile`, which every mechanism carries.
* **A gate passed on another row's evidence.** The new V03/V04 check read
  `ctx["autostart"]`, a key already holding V08's diagnostics component. It
  found no `failed` field, concluded zero failures and reported PASS — the exact
...
| V01 clean install · V13 production re-index · V15 destructive uninstall | each needs a machine that can be broken, or would modify this user's working install | **D-1** — since closed against sandboxes: a built wheel in a clean interpreter, the real migrated corpus, and an install/uninstall/sweep cycle |
